window.onload = function() {
    ...
    var lastMouseX = 0;
    var lastMouseY = 0;
    var lastUpdateTime = 0;       // Time of the last mouse move
    var qinc = new Quaternion();  // Incremental rotation quaternion
    var spinning = false;         // Flag to control spinning state
    var lastMousePosition = { x: 0, y: 0 }; // Store last mouse position
    ...
    canvas.addEventListener('mouseup', function(event) {
        ...
        // Stop spinning if mouse position hasn’t changed
        if (currentMouseX === lastMousePosition.x && currentMouseY === lastMousePosition.y) {
            spinning = false;
            qinc = new Quaternion(); // Reset incremental rotation
        }
    });
    ...
    canvas.addEventListener('mousemove', function(event) {
        ...
        qinc = new Quaternion().multiply(rotationQuatX).multiply(rotationQuatY);
        
        spinning = true; // Flag spinning
    ...
    function render() {
        ...
        if (spinning) {
            // Gradually reduce qinc to simulate friction
            qinc.w *= 0.98;  // Decay factor, adjust for smooth slowdown
            qinc.x *= 0.98;
            qinc.y *= 0.98;
            qinc.z *= 0.98;
    
            if (Math.abs(qinc.w) < 0.0001) { 
                qinc = new Quaternion(); // Stop completely when small enough
                spinning = false;
            }
        }
    
        // Apply incremental quaternion to current quaternion
        currentQuaternion.multiply(qinc).normalize();
